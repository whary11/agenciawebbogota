<template>
    ...
    <div class="col-md-12">
        ...
        <div class="" v-else>
                <div>
                    <div v-if="error_mensaje.estado !=null " :class="{'alert alert-primary':error_mensaje.estado, 'alert alert-danger':!error_mensaje.estado, }" role="alert">
                       <span > {{error_mensaje.mensaje}} </span>
                    </div>
                    <div class="card-header bg-white">
                        Hay {{ticket.conversaciones.length}} conversaciones relacionadas al Ticket # {{num_ticket}}
                    </div>
                    <form @submit.prevent="sendConversacion()">
                        <div class="input-group mb-3 shadow-sm p-3 mb-5 bg-white rounded">
